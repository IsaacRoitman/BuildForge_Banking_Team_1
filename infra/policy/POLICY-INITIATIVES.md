...
| Exported data types | Assessments (+ snapshot), SubAssessments (+ snapshot, "security findings"), Alerts, SecureScores (+ snapshot), SecureScoreControls (+ snapshot), RegulatoryComplianceAssessment (+ snapshot) — 11 event sources. `Assessments`/`AssessmentsSnapshot` include the `type Contains Microsoft.Security/assessments` ruleSet and `Alerts` includes all 4 severity ruleSets (low/medium/high/informational) — required for the portal's Continuous Export blade to correctly show each data type checkbox as selected; sources without matching ruleSets deploy fine but render as unchecked in the portal UI even though the automation is active |
